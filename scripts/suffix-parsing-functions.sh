...
parse_package_suffix() {
  ...
  if [[ "$filename" =~ [+]([a-z]+)[+]([a-z]+)\.deb$ ]]; then
    ...
    # No suffix found - default to main component
    eval "$distro_var=\"any\""
...
validate_suffix() {
  ...
  if [[ ! " ${VALID_DISTROS[@]} " =~ " ${distro} " ]]; then
    echo "Warning: Unknown distro '$distro' (valid: ${VALID_DISTROS[*]})" >&2
    valid=1
  fi

  # Check if component is in valid list
  if [[ ! " ${VALID_COMPONENTS[@]} " =~ " ${component} " ]]; then
    echo "Warning: Unknown component '$component' (valid: ${VALID_COMPONENTS[*]})" >&2
    valid=1
  fi
