...
# Constants
REPO_URL = 'https://apt.halos.fi'
KEYRING_PATH = '/usr/share/keyrings/halos.gpg'
SUPPORTED_ARCHITECTURES = ['arm64', 'armhf', 'all']

UNSTABLE_WARNING = '''
                <div class="warning-box">
                    <strong>Warning: Unstable Channel:</strong> Contains latest packages from main branch. May include untested changes. Use stable for production systems.
                </div>
'''
...
class Distribution:
    ...
    def components(self) -> List[str]:
        ...
        # Return in preferred order: main first, then others
        ordered = []
        if 'main' in found:
            ordered.append('main')
            found.remove('main')
...
def parse_packages_file(packages_file: Path, component: str = 'main') -> List[Package]:
    ...
    Args:
        ...
        component: Component name (main, etc.) for tracking
    """
    packages = []
    current_package = {}
    last_key = None
...
def scan_distributions(repo_dir: Path) -> List[Distribution]:
    ...
    Scans all components found in each distribution.
    """
    distributions = []
    ...
    for dist_path in sorted(dists_dir.iterdir()):
        ...
        # Sort components: main first, then others alphabetically
        def component_sort_key(c):
            return (0, '') if c == 'main' else (1, c)
        ...
        for pkg in all_packages:
            ...
            if key not in unique_packages:
                ...
                if pkg.architecture not in existing.all_architectures:
                    ...
                    existing.all_architectures.sort()
...
def render_distribution_summary_card(dist: Distribution) -> str:
    ...
    parts.append(f'\n                        <div class="command-block">echo "deb [signed-by={html.escape(KEYRING_PATH)}] {html.escape(REPO_URL)} {html.escape(dist.name)} {html.escape(components_str)}" | sudo tee -a /etc/apt/sources.list.d/halos.list</div>')
    parts.append('\n                    </div>')
    parts.append('\n                </details>')
...
<head>
    ...
    <title>HaLOS APT Repository</title>
...
<body>
    ...
    header += '''
        <header>
            <h1>HaLOS APT Repository</h1>
            <p class="subtitle">Debian packages for the HaLOS operating system</p>
        </header>
'''
    return header
...
def render_main_index(distributions: List[Distribution], gpg_fingerprint: str) -> str:
    ...
    html_parts.append(f'''
        <div class="info-box">
            ...
            <p>Add the HaLOS repository to your system:</p>
            <div class="command-block">curl -fsSL {html.escape(REPO_URL)}/halos-apt-key.asc | sudo gpg --dearmor -o {html.escape(KEYRING_PATH)}
echo "deb [signed-by={html.escape(KEYRING_PATH)}] {html.escape(REPO_URL)} <distribution> main" | sudo tee -a /etc/apt/sources.list.d/halos.list
sudo apt update</div>
            <p style="margin-top: 10px;"><small>Replace <code>&lt;distribution&gt;</code> with your desired distribution (e.g., <code>trixie-stable</code>)</small></p>
        </div>
''')
    ...
    if distributions:
        ...
        html_parts.append('\n            <h2>HaLOS Packages</h2>')
        html_parts.append('\n            <p style="margin-bottom: 20px; color: #4a5568;">HaLOS packages for different Debian/Raspberry Pi OS releases</p>')
    ...
    html_parts.append(f'''
        <div class="info-box">
            ...
            <p style="margin-top: 10px;">
                Download: <a href="halos-apt-key.asc">halos-apt-key.asc</a><br>
