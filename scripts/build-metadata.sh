...
build_release() {
  ...
  local components=""
  if [ -f "$dist_path/main/binary-arm64/Packages" ] || [ -f "$dist_path/main/binary-armhf/Packages" ] || [ -f "$dist_path/main/binary-all/Packages" ]; then
    components="main"
  fi
  ...
  if [[ "$dist" =~ ^([a-z0-9]+)-(stable|unstable)$ ]]; then
    ...
    SUITE="$dist"
    CODENAME="$dist"
    DESC="HaLOS APT Repository - $dist"
  fi
